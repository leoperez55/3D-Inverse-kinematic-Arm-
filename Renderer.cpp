...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	//End Effectors GUI
	gui_1->addGroup("End Effectors");

	gui_1->addVariable("Move towards target: ", m_bone_animation->moveTowardsTarget);

	gui_1->addVariable("targetEndEffector gX", m_bone_animation->targetEndEffector[0]);
	gui_1->addVariable("targetEndEffector  gY", m_bone_animation->targetEndEffector[1]);
	gui_1->addVariable("targetEndEffector  gZ", m_bone_animation->targetEndEffector[2]);


	currentEndEffector_X = gui_1->addVariable("Current eX", m_bone_animation->endEffector.x);
	currentEndEffector_Y = gui_1->addVariable("Current eY", m_bone_animation->endEffector.y);
	currentEndEffector_Z = gui_1->addVariable("Current eZ", m_bone_animation->endEffector.z);

	gui_1->addGroup("Bone Rotation");
	
	yellowBone_x_widget = gui_1->addVariable("Rotate X1", m_bone_animation->yellowAng_X);
	yellowBone_y_widget = gui_1->addVariable("Rotate Y1", m_bone_animation->yellowAng_Y);
	yellowBone_z_widget = gui_1->addVariable("Rotate Z1", m_bone_animation->yellowAng_Z);

	pinkBone_x_widget = gui_1->addVariable("Rotate X2", m_bone_animation->purpleAng_X);
	pinkBone_y_widget = gui_1->addVariable("Rotate Y2", m_bone_animation->purpleAng_Y);
	pinkBone_z_widget = gui_1->addVariable("Rotate Z2", m_bone_animation->purpleAng_Z);

	blueBone_x_widget = gui_1->addVariable("Rotate X3", m_bone_animation->blueAng_X);
	blueBone_y_widget = gui_1->addVariable("Rotate Y3", m_bone_animation->blueAng_Y);
	blueBone_z_widget = gui_1->addVariable("Rotate Z3", m_bone_animation->blueAng_Z);
	
	
	gui_1->addButton("Reset Bone Rotations", [&]() {
		
		m_bone_animation->oldFwdKinematics(0, 0, 0, 0, 0, 0, 0, 0, 0);
...
void Renderer::init()
{
	...
	this->m_window = glfwCreateWindow(m_camera->width/1.5, m_camera->height/1.2, "Assignment 4", nullptr, nullptr);
	...
	//Call for the start of the calculations in BoneAnimation
...
void Renderer::display(GLFWwindow* window)
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		yellowBone_x_widget->setValue(m_bone_animation->degreesAdded[6]);
		pinkBone_x_widget->setValue(m_bone_animation->degreesAdded[7]);
		blueBone_x_widget->setValue(m_bone_animation->degreesAdded[8]);

		yellowBone_y_widget->setValue(m_bone_animation->degreesAdded[3]);
		pinkBone_y_widget->setValue(m_bone_animation->degreesAdded[4]);
		blueBone_y_widget->setValue(m_bone_animation->degreesAdded[5]);


		yellowBone_z_widget->setValue(m_bone_animation->degreesAdded[0]);
		pinkBone_z_widget->setValue(m_bone_animation->degreesAdded[1]);
		blueBone_z_widget->setValue(m_bone_animation->degreesAdded[2]);


		currentEndEffector_X->setValue(m_bone_animation->endEffector.x);
		currentEndEffector_Y->setValue(m_bone_animation->endEffector.y);
		currentEndEffector_Z->setValue(m_bone_animation->endEffector.z);
...
void Renderer::draw_bones(Shader& shader, Bone_Animation* m_bone_animation)
{
	...
	// Draw yellow bone////////////////////////////////////////////////////////////////////////
	glm::mat4 yellow_bone_obj_mat = m_bone_animation->get_yellow_bone_obj_mat();
	...
	// Draw pink bone////////////////////////////////////////////////////////////////////////
	glm::mat4 pink_bone_obj_mat = m_bone_animation->get_pink_bone_obj_mat();
	...
	// Draw blue bone////////////////////////////////////////////////////////////////////////
	glm::mat4 blue_bone_obj_mat = m_bone_animation->get_blue_bone_obj_mat();
	...
	///////Draw Target EndEffector//////////////////////////////////////////////////////////////////////////
	glm::mat4 target_obj_mat = m_bone_animation->get_targetEndEffector_obj_mat();
